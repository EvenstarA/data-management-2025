...
In reality, we only need columns 2 to 5 from the quadrat data frame; so we'll only tack those onto the main pitfall data frame. To do this, we'll extract the columns we want into a new variable, then bind them to the pitfall data. ***This only works well because we haven't jumbled the order of either data set, so if we copy the columns directly across, they will correspond to the correct sites/pitfall traps.***
...
Now we'll run correlation tests on quadrat variables and species richness:
...
From this we can see that the relationship between live vegetation cover, bare ground cover and species richness is positive, but not statistically significant. The relationship between leaf litter and rocks/stones is negative but not statistically significant.
...
Here, we can see some interesting results!! The correlation between live vegetation and total abundance is positive and statistically significant! Also, leaf litter and total abundance is negatively correlated and statistically significant. However, both bare ground and rocks/stones aren't statistically significant and are negatively and positively correlated, respectively.

But, keep in mind that correlation does not imply causation!
